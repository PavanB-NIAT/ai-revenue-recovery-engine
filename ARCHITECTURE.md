...
## 12. Architectural Extension Point

The transaction-level recovery pipeline is intentionally designed as an autonomous unit of execution. This modular structure provides a clean **extension point** where a future higher-level intelligence layer can sit **above** the recovery engine:

```mermaid
flowchart TD
    subgraph FUTURE ["Higher-Level Orchestration Layer (Future Extension Point)"]
        F1[Portfolio-Level Opportunity Allocation]
        F2[Uncertainty-Aware Capital Allocation]
        F3[Multi-Merchant Resource Budgeting]
    end

    subgraph CORE ["Core Autonomous Transaction Engine (Current Frozen Baseline)"]
        C1[AutonomousRecoveryPipeline: Transaction A]
        C2[AutonomousRecoveryPipeline: Transaction B]
        C3[AutonomousRecoveryPipeline: Transaction C]
    end
...
- **Transaction Engine Autonomy**: The underlying pipeline accepts a transaction and contextual snapshot, evaluates local EV, and executes bounded recovery.
- **Portfolio-Level Extension**: A future supervisory controller can manage global recovery budgets, prioritize high-tier merchants during systemic bank outages, or apply global throttling without modifying core transaction-level recovery logic.
